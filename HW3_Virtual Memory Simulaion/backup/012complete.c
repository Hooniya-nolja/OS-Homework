...
void twoLevelVMSim(int numProcess, int nFrame, int firstLevelBits) {
	int i, j, k, update, frameId, scanfValue;
	int hit = 0;
	int frameCount = 0;
	unsigned addr, firstAddr, secondAddr, deleteFA, deleteSA;
	char rw;
	...
	while(1) {
		for (j=0; j < numProcess; j++) {
			...
			addr = ((addr/16)/16)/16;
			firstAddr = addr >> (20 - firstLevelBits);
			secondAddr = addr - (firstAddr << (20 - firstLevelBits));
			// printf("firstAddr : %x, secondAddr : %x\n", firstAddr, secondAddr);
			procTable[j].ntraces++;
			...
			if (hit == 0) {	// Page Fault !!
				...
				if (procTable[j].firstLevelPageTable[firstAddr] != 1) {
					procTable[j].secondLevelPageTable[firstAddr] = malloc(sizeof(int) * 100001);
					procTable[j].firstLevelPageTable[firstAddr] = 1;
				...
				if (frameCount < nFrame) {
					...
					procTable[j].secondLevelPageTable[firstAddr][secondAddr] = pageFrame[frameCount].id;
					frameCount++;
				} else {
					frameId = pageFrameQHead.next->id;
					deleteFA = pageFrame[frameId].firstPageTableAddr >> (20 - firstLevelBits);
					deleteSA = pageFrame[frameId].firstPageTableAddr - (deleteFA << (20 - firstLevelBits));

					// printf("FA : %x, SA : %x\n", deleteFA, deleteSA);
					procTable[pageFrame[frameId].pid]
						.secondLevelPageTable[deleteFA][deleteSA] = -1;
					...
					pageFrame[frameId].pid = j;
					pageFrame[frameId].firstPageTableAddr = addr;
					procTable[j].secondLevelPageTable[firstAddr][secondAddr] = pageFrame[frameId].id;
				}
			} else {	// Page Hit !!
				if (pageFrame[update].next != &pageFrameQHead) {
					pageFrame[update].prev->next = pageFrame[update].next;
...
int main(int argc, char *argv[]) {
	...
	for(i = 0; i < numProcess; i++) {
		...
		if (simType == 2) {
			procTable[i].secondLevelPageTable = malloc(sizeof(int *) * 1000001);
		}
		// procTable[i].secondLevelPageTable = (int *)malloc(sizeof(int) * 1000001);
	}
