...
void oneLevelVMSim(int numProcess, int nFrame) {
	...
	while(1) {
		...
		for (j=0; j < numProcess; j++) {
			...
			for (k=0; k < procTable[j].ntraces; k++) { // sizeof(procTable[j].firstLevelPageTable)
				if (procTable[j].firstLevelPageTable[k] == addr) {
					tableIndex = k;
				}
			}
			...
			if (tableIndex == -1) {	
				...
				if (frameCount < nFrame) {
					...
					pageFrame[frameCount].firstPageTableAddr = procTable[j].numPageTable;
					...
					procTable[j].firstLevelPageTable[procTable[j].numPageTable] = addr;
					frameCount++;
				} else {
					...
					procTable[pageFrame[fifo].pid].firstLevelPageTable[pageFrame[fifo]
						.firstPageTableAddr] = -1;
					pageFrame[fifo].pid = procTable[j].pid;
					pageFrame[fifo].firstPageTableAddr = procTable[j].numPageTable;
					procTable[j].firstLevelPageTable[procTable[j].numPageTable] = addr;
					fifo++;
				}
				procTable[j].numPageTable++;
			} else {	// page hit
				printf("*********** Page hit *************\n");
